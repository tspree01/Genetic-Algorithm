...
class Model {
	...
	Model clone(Controller c, Object symbol) {
		...
		for (int i = 0; i < sprites_blue.size(); i++) { m.sprites_blue.add(sprites_blue.get(i).clone(m)); }

		m.sprites_red = new ArrayList<Sprite>();

		for (int i = 0; i < sprites_red.size(); i++) { m.sprites_red.add(sprites_red.get(i).clone(m)); }
		...
		for (int i = 0; i < bombs_throwing.size(); i++) { m.bombs_throwing.add(bombs_throwing.get(i).clone()); }
		m.bombs_flying = new ArrayList<Bomb>();

		for (int i = 0; i < bombs_flying.size(); i++) { m.bombs_flying.add(bombs_flying.get(i).clone()); }
		m.bombs_exploding = new ArrayList<Bomb>();

		for (int i = 0; i < bombs_exploding.size(); i++) { m.bombs_exploding.add(bombs_exploding.get(i).clone()); }
	...
	void update() {
		...
		for (int i = 0; i < sprites_blue.size(); i++) { sprites_blue.get(i).update(); }

		// Update the red agents
		for (int i = 0; i < sprites_red.size(); i++) { sprites_red.get(i).update(); }
		...
		for (int i = 0; i < bombs_throwing.size(); i++) { bombs_flying.add(bombs_throwing.get(i)); }
		...
		for (int i = 0; i < bombs_flying.size(); i++) {
			...
			if (b.position > b.distance) {
				...
				// Damage nearby flags
				if (b.doesHit(XFLAG, YFLAG)) { energy_blue -= BOMB_DAMAGE_TO_FLAG; }
				if (b.doesHit(XFLAG_OPPONENT, YFLAG_OPPONENT)) { energy_red -= BOMB_DAMAGE_TO_FLAG; }

				// Damage nearby sprites
				for (int j = 0; j < sprites_blue.size(); j++) { sprites_blue.get(j).onDetonation(b, false); }
				for (int j = 0; j < sprites_red.size(); j++) { sprites_red.get(j).onDetonation(b, true); }
			}
		}
	}
	...
	float getBombTargetX(int bomb) {
		if (sprites_self == sprites_blue) { return bombs_flying.get(bomb).xEnd; }
		else { return XMAX - bombs_flying.get(bomb).xEnd; }
	}

	float getBombTargetY(int bomb) {
		if (sprites_self == sprites_blue) { return bombs_flying.get(bomb).yEnd; }
		else { return YMAX - bombs_flying.get(bomb).yEnd; }
	}
	...
	void throwBomb(int sprite, float x, float y) {
		Sprite s = sprites_self.get(sprite);
		if (s.energy < BOMB_COST || s.cooldown > 0) { return; }
		s.energy -= BOMB_COST;
	...
	class Sprite {
		...
		void update() {
			...
			else { this.energy = Math.min(1.0f, this.energy + ENERGY_RECHARGE_RATE); }
		...
		void onDetonation(Bomb b, boolean red) {
			if (! b.doesHit(red ? XMAX - x : x, red ? YMAX - y : y) || energy < 0.0f) { return; }
